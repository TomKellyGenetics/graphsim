...
make_state_matrix <- function(graph, state = "activating"){
  ...
  if(length(edges) > 0){
    rows <- unlist(apply(edges, 1, function(x) grep(unlist(as.list(x)[1], use.names = F), names(V(graph)))))
    cols <- unlist(apply(edges, 1, function(x) grep(unlist(as.list(x)[2], use.names = F), names(V(graph)))))
