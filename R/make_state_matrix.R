...
make_state_matrix <- function(graph, state){
  ...
  for(ii in 1:length(rows)){
    ...
    sub_edge <- get.edgelist(graph)[setdiff(c(1:length(E(graph))),intersect(grep(names(V(g))[rows[ii]], as.matrix(get.edgelist(graph))[,1]),  grep(names(V(g))[cols[ii]], as.matrix(get.edgelist(graph))[,2]))),]
    sub_graph <- graph.edgelist(sub_edge)
    clust <- clusters(sub_graph)
    down_ind <- grep(clust$membership[match(names(V(g))[cols[ii]], names(clust$membership))], clust$membership) #find downstream genes
