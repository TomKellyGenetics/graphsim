...
make_state_matrix <- function(graph, state){
  ...
  edges <- as_adjmat(get.edgelist(graph)[grep(-1, state),])
  ...
  for(ii in 1:length(rows)){
    ...
    sub_edge <- get.edgelist(graph)[setdiff(c(1:length(E(graph))),intersect(grep(names(V(graph))[rows[ii]], as_adjmat(get.edgelist(graph))[,1]),  grep(names(V(graph))[cols[ii]], as_adjmat(get.edgelist(graph))[,2]))),]
