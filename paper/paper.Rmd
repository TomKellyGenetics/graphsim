...
interactions [@Schaffter2011, @Saelens2019]. Dynamical models using differential equations have been employed,
such as by GeneNetWeaver [@Schaffter2011], to generate simulated datasets
...
to use this data [@Zappia2017, @Saelens2019].

Splatter [@Zappia2017] was developed to model single-cell data and 
SERGIO [@Dibaeinia2019] and dyngen [@Cannoodt2020] build on this to
include gene regulatory networks and multimodality respectively.

These methods have been designed based on known or modelled reaction
states and added stochasticity to determinisitic relationships.
However, it is computationally-intensive to model these reactions
at scale over iterations in benchmarking. In some cases, it is only
necessary to model the statistical variability of RNA-Seq data, 

GeneNetWeaver @Schaffter2011 - benchmarking of gene regulatory network inference methods
Splatter @Zappia2017 - simulated single-cell RNA-Seq data (gamma-poisson)

dyngen @Cannoodt2020 - single-cell multimodal -  Gillespie’s stochastic simulation at single-molecule level

SERGIO @Dibaeinia2019 - single-cell gene expression (noise with poisson)
